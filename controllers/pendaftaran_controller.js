const Pendaftaran = require("../models/pendaftaran_model.js");
...
exports.getAll = (req,res) => {
    Pendaftaran.getAll((err,data) => {
        if(err){
            res.status(500).send({
                message: err.message || "Some error occured while retrieving pendaftaran."
            })
        }else{
            res.send(data);
        }
...
exports.deleteById = (req,res) => {
    Pendaftaran.deleteById(req.params.id,(err,data) => {
        ...
        if(err){
            res.status(500).send({
                message: err.message || "Some error occured while deleting the pendaftaran."
            })
        }else{
            res.send({
...
exports.updateById = (req,res) => {
    ...
    Pendaftaran.updateById(id,pendaftaran,(err,data) => {
        if(err){
            res.status(500).send({
                message: err.message || "Some error occured while updating pendaftaran with id = " + id
            })
        }else{
            res.send(data);
        }
