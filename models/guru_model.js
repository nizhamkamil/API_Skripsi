...
Guru.updateById = (id,guru,result) => {
    let query = "UPDATE guru SET nama = ?, username = ?, password = ?, alamat = ?, email = ?, no_telepon = ?, agama = ?, kewarganegaraan = ?, jenis_kelamin = ?, status_nikah = ? WHERE id_guru = ?";
    sql.query(query, [guru.nama, guru.username, guru.password, guru.alamat, guru.email, guru.no_telepon, guru.agama, guru.kewarganegaraan, guru.jenis_kelamin, guru.status_nikah, id], (err,res) => {
        if(err){
            console.log("error: ", err);
            result(null,err);
            return;
