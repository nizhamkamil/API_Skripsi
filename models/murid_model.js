...
const Murid = function (murid) {
  ...
  this.tempat_lahir = murid.tempat_lahir;
  this.tipe_pembelajaran = murid.tipe_pembelajaran;
};
...
Murid.selectById = (id, result) => {
  sql.query(
    "SELECT DISTINCT m.* FROM murid m JOIN jadwal_pembelajaran j ON m.id_murid = j.id_murid JOIN guru g ON g.id_guru = j.id_guru WHERE g.id_guru = ?",
    id,
    (err, res) => {
      if (err) {
        console.log("error: ", err);
